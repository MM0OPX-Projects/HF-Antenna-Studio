...
test("0.1/0.25/0.5/1/2 wavelength perfect-ground cases match recorded nec2c envelopes", async ({ page }) => {
  ...
  for (const reference of HEIGHT_REFERENCES) {
    ...
    await expect(page.getByTestId("height-results"), `${reference.height}λ result`).toBeVisible({ timeout: 30_000 });
