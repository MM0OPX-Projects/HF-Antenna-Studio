...
import { COMPARISON_COLORS, COMPARISON_PRESETS, FAMILY_PARAMETERS, clonePreset, comparisonConditionKey, comparisonConditionWarnings, comparisonDefinitionKey, elevationBearingMode, enabledComparisonSlots, isComparisonSlotEnabled, savedProjectConditionMode, validateComparisonDefinition, validateComparisonRadialCounts } from "../features/model-comparison/model";
...
export function ModelComparisonPage() {
  const { projects } = useProjectSession();
  ...
  const validationErrors = useMemo(() => [
    ...(enabledComparisonSlots(definitions).length < 2 ? ["Enable at least two model slots before running a comparison."] : []),
    ...definitions.flatMap((definition, index) => isComparisonSlotEnabled(definition) ? validateComparisonDefinition(definition).map((message) => `Model ${index + 1}: ${message}`) : []),
    ...validateComparisonRadialCounts(enabledComparisonSlots(definitions), conditions),
    ...
    ...(!Number.isFinite(conditions.azimuthElevationDeg) || conditions.azimuthElevationDeg < 0 || conditions.azimuthElevationDeg > 90 ? ["Azimuth-cut elevation must be from 0° to 90° above the horizon. Each model interpolates between adjacent solved NEC rows when needed."] : []),
    ...(bearingMode === "common" && (!Number.isFinite(conditions.elevationBearingDeg) || conditions.elevationBearingDeg < 0 || conditions.elevationBearingDeg > 350 || conditions.elevationBearingDeg % 10 !== 0) ? ["Elevation-cut bearing must be a 10° increment from 0° to 350° so every family uses the same solved grid sample."] : []),
  ...
  ], [bearingMode, conditions, definitions, sweep]);
  ...
  const elevationSeries: PolarSeries[] = compatibleResults.map((result) => ({ id: result.slotId, label: `${result.label} · ${result.elevationBearingDeg.toFixed(0)}° bearing`, color: result.color, points: result.elevationPattern }));
  ...
  const eligibleProjects = useMemo(() => projects.filter((record) => record.project.mode === "simulator" || record.project.mode === "editor" || record.project.mode === "module"), [projects]);
  const chooseSource = (definition: ComparisonSlotDefinition, value: string) => {
    if (value.startsWith("snapshot:")) return;
    if (value.startsWith("builtin:")) { chooseFamily(definition.id, value.slice(8) as ComparisonFamily); updateSlot(definition.id, { source: "builtin", savedProject: undefined, savedConditionMode: "common" }); return; }
    const record = eligibleProjects.find((candidate) => candidate.id === value.slice(8));
    if (!record) return;
    updateSlot(definition.id, { source: "saved-project", savedConditionMode: "common", savedProject: { projectId: record.id, projectName: record.name, projectRevision: record.revision, project: structuredClone(record.project) } });
  };

  const runAll = async () => {
    if (validationErrors.length) return;
    const activeDefinitions = definitions.flatMap((definition, index) => isComparisonSlotEnabled(definition) ? [{ definition, index }] : []);
    controllerRef.current?.abort();
    const controller = new AbortController(); controllerRef.current = controller;
    setPhase("running"); setProgress(0); setError(null); setResults([]);
    const completed: ComparisonResult[] = [];
    try {
      for (let activeIndex = 0; activeIndex < activeDefinitions.length; activeIndex += 1) {
        const { definition, index } = activeDefinitions[activeIndex]!;
        const result = await runComparisonSlot(definition, conditions, { ...sweep, referenceOhms: conditions.referenceImpedanceOhm }, { signal: controller.signal, colorIndex: index });
        if (controller.signal.aborted) throw new Error("Comparison cancelled.");
        completed.push(result); setResults([...completed]); setProgress(activeIndex + 1);
      }
      setPhase("complete");
    } catch (caught) {
    ...
    <div className="mx-auto max-w-[1600px] space-y-4">
      ...
      <Card className="space-y-4 p-4" data-testid="comparison-conditions"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-semibold">Common comparison conditions</h2><p className="text-xs text-text-secondary">These controls apply to every model in the next run. Existing snapshots are retained only to expose a mismatch.</p></div><span className="text-xs text-accent" data-testid="comparison-condition-summary">{conditions.frequencyMhz.toFixed(3)} MHz · {groundText} · {conditions.referenceImpedanceOhm} Ω</span></div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
          ...
          <NumericInput label="Azimuth cut elevation (°)" value={conditions.azimuthElevationDeg} min={0} max={90} step={0.1} testId="comparison-azimuth-elevation" onChange={(azimuthElevationDeg) => updateConditions({ azimuthElevationDeg })} />
          <label className="space-y-1"><span className="text-[10px] uppercase tracking-wide text-text-secondary">Elevation-cut bearing mode</span><select data-testid="comparison-elevation-bearing-mode" value={bearingMode} onChange={(event) => updateConditions({ elevationBearingMode: event.target.value as "common" | "strongest" })} className="w-full rounded-md border border-border bg-background px-2 py-2 text-sm"><option value="common">Common compass bearing</option><option value="strongest">Each model's strongest bearing</option></select></label>
          <label className={`space-y-1 ${bearingMode === "strongest" ? "opacity-55" : ""}`}><span className="text-[10px] uppercase tracking-wide text-text-secondary">Elevation bearing (°)</span><input data-testid="comparison-elevation-bearing" type="number" value={conditions.elevationBearingDeg} min={0} max={350} step={10} disabled={bearingMode === "strongest"} onChange={(event) => updateConditions({ elevationBearingDeg: event.target.valueAsNumber })} className="w-full rounded-md border border-border bg-background px-2 py-2 font-mono text-sm outline-none focus:border-accent disabled:cursor-not-allowed" /></label>
        ...
        <GroundRadialWorkflowControls settings={conditions.radialSystems} showVertical={definitions.some((definition) => isComparisonSlotEnabled(definition) && definition.source !== "saved-project" && definition.family === "vertical")} showPhased={definitions.some((definition) => isComparisonSlotEnabled(definition) && definition.source !== "saved-project" && definition.family === "phased-array")} prefix="comparison" onChange={(radialSystems) => updateConditions({ radialSystems })} />
      </Card>

      <section className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-semibold">Comparison model slots</h2><p className="text-xs text-text-secondary">Enable two, three, or all four models. Disabled slots retain their settings and can be re-enabled at any time. Frequency-based dimensions are starting values, not resonance guarantees.</p></div><div className="flex flex-wrap gap-1">{(["mixed", "dipole", "vertical", "phased", "yagi"] as const).map((name) => <button key={name} type="button" data-testid={`comparison-preset-${name}`} onClick={() => applyPreset(name)} className="rounded border border-border px-2 py-1 text-xs capitalize text-text-secondary hover:border-accent">{name === "mixed" ? "Mixed example" : `${name} example`}</button>)}</div></div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{definitions.map((definition, index) => { const meta = FAMILY_PARAMETERS[definition.family]; const saved = definition.source === "saved-project"; const enabled = isComparisonSlotEnabled(definition); const snapshotValue = `snapshot:${definition.savedProject?.projectId}:${definition.savedProject?.projectRevision}`; const savedProjectMode = (definition.savedProject?.project as { mode?: string } | undefined)?.mode; const supportsSavedConditions = savedProjectMode === "editor" || savedProjectMode === "simulator"; return <Card key={definition.id} className={`space-y-3 p-3 ${enabled ? "" : "opacity-60"}`} data-testid={`comparison-slot-${index + 1}`}><div className="flex items-center justify-between"><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" data-testid={`comparison-enabled-${index + 1}`} checked={enabled} onChange={(event) => { updateSlot(definition.id, { enabled: event.target.checked }); setResults([]); setPhase("idle"); setProgress(0); }} />Model {index + 1}</label><span className="h-2.5 w-8 rounded" style={{ background: COMPARISON_COLORS[index] }} /></div><label className="space-y-1"><span className="text-[10px] uppercase tracking-wide text-text-secondary">Antenna source</span><select disabled={!enabled} data-testid={`comparison-family-${index + 1}`} value={saved ? snapshotValue : `builtin:${definition.family}`} onChange={(event) => chooseSource(definition, event.target.value)} className="w-full rounded-md border border-border bg-background px-2 py-2 text-sm">{saved && <optgroup label="Captured snapshot"><option value={snapshotValue}>{definition.savedProject?.projectName} · revision {definition.savedProject?.projectRevision}</option></optgroup>}<optgroup label="Standard models"><option value="builtin:dipole">Horizontal dipole</option><option value="builtin:vertical">Quarter-wave vertical</option><option value="builtin:yagi">Three-element Yagi</option><option value="builtin:phased-array">Two-element phased array</option></optgroup>{eligibleProjects.length > 0 && <optgroup label="Saved projects">{eligibleProjects.map((record) => <option key={record.id} value={`project:${record.id}`}>{record.name} · {record.project.mode === "module" ? moduleProjectDefinition(record.project.module?.moduleId ?? "")?.label ?? "Module" : record.project.mode === "editor" ? "Wire Editor" : "Design"}</option>)}</optgroup>}</select></label>{saved ? <div className="rounded border border-border bg-background/60 p-2 text-xs"><strong>{definition.savedProject?.projectName}</strong><p className="mt-1 text-text-secondary">Immutable revision {definition.savedProject?.projectRevision} snapshot.</p>{supportsSavedConditions ? <label className="mt-2 block space-y-1"><span className="text-[10px] uppercase tracking-wide text-text-secondary">Saved-project conditions</span><select data-testid={`comparison-saved-condition-mode-${index + 1}`} disabled={!enabled} value={savedProjectConditionMode(definition)} onChange={(event) => updateSlot(definition.id, { savedConditionMode: event.target.value as "common" | "saved" })} className="w-full rounded border border-border bg-surface px-2 py-1 text-xs"><option value="common">Use common comparison conditions</option><option value="saved">Use saved model conditions</option></select></label> : <p className="mt-2 text-text-secondary">This specialist snapshot uses common comparison conditions.</p>}<p className="mt-2 text-text-secondary">{savedProjectConditionMode(definition) === "saved" ? "Saved frequency, ground and GE geometry-ground settings are preserved; this is an exact reproduction, not a common-condition comparison." : "Common frequency and ground are applied only to the comparison copy."}</p>{eligibleProjects.some((record) => record.id === definition.savedProject?.projectId && record.revision !== definition.savedProject?.projectRevision) && <button type="button" className="mt-2 text-accent hover:underline" onClick={() => chooseSource(definition, `project:${definition.savedProject?.projectId}`)}>Refresh from latest saved revision</button>}</div> : <><NumericInput label={`${meta.parameterLabel} ${meta.unit ? `(${meta.unit})` : ""}`} value={definition.parameterValue} min={meta.min} max={meta.max} step={meta.step} testId={`comparison-parameter-${index + 1}`} onChange={(parameterValue) => updateSlot(definition.id, { parameterValue })} /><p className="text-[10px] leading-relaxed text-text-secondary">{definition.family === "phased-array" ? `Ideal relative-current mode; ${radialWorkflowSummary(conditions.radialSystems, "phased-array")}; no single input impedance exists.` : definition.family === "vertical" ? radialWorkflowSummary(conditions.radialSystems, "vertical") : "Dimensions rescale from the common frequency; height remains exact."}</p></>}</Card>; })}</div>
      </section>
