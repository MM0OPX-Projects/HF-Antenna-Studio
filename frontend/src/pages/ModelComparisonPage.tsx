...
import { COMPARISON_COLORS, COMPARISON_PRESETS, FAMILY_PARAMETERS, clonePreset, comparisonConditionKey, comparisonConditionWarnings, comparisonDefinitionKey, enabledComparisonSlots, isComparisonSlotEnabled, validateComparisonDefinition, validateComparisonRadialCounts } from "../features/model-comparison/model";
...
export function ModelComparisonPage() {
  ...
  const chooseSource = (definition: ComparisonSlotDefinition, value: string) => {
    ...
    if (value.startsWith("builtin:")) { chooseFamily(definition.id, value.slice(8) as ComparisonFamily); updateSlot(definition.id, { source: "builtin", savedProject: undefined }); return; }
    const record = eligibleProjects.find((candidate) => candidate.id === value.slice(8));
    if (!record) return;
    updateSlot(definition.id, { source: "saved-project", savedProject: { projectId: record.id, projectName: record.name, projectRevision: record.revision, project: structuredClone(record.project) } });
  };
