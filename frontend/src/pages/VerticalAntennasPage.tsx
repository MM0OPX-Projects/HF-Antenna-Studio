  ...
  return <div className="flex h-dvh flex-col bg-background"><Navbar /><main className="flex-1 overflow-y-auto"><div className="mx-auto max-w-[1540px] space-y-5 px-3 py-5 sm:px-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]"><aside className="space-y-4">
      ...
      {model.configuration !== "ground-mounted-ideal" && <Card className="space-y-4 p-4"><h2 className="text-sm font-semibold">{model.radials.representation === "explicit-wires" ? "Explicit radial wires" : "Simplified NEC screen"}</h2>
        <VerticalSliderField label="Radial number" description={model.radials.representation === "explicit-wires" ? "Actual NEC wires meeting at the feed." : "Density parameter on the NEC GN card."} value={model.radials.count} min={model.radials.representation === "explicit-wires" ? 2 : 4} max={model.radials.representation === "explicit-wires" ? 64 : 128} step={1} unit="count" decimals={0} testId="vertical-radial-count" onChange={(value) => updateRadials({ count: Math.round(value) })} />
        ...
        {model.radials.representation === "explicit-wires" && <VerticalSliderField label="Radial droop" description="Angle below horizontal; endpoint clearance is checked." value={model.radials.droopAngleRad * 180 / Math.PI} min={0} max={60} step={1} unit="deg" decimals={0} testId="vertical-radial-angle" onChange={(value) => updateRadials({ droopAngleRad: value * Math.PI / 180 }, true)} />}
      </Card>}
      ...
      <Card className="space-y-3 p-4">
        ...
        <div className="rounded border border-blue-500/25 bg-blue-500/10 p-3 text-[10px] leading-relaxed text-blue-600 dark:text-blue-300" data-testid="vertical-ground-explanation">{model.configuration === "ground-mounted-ideal" ? "Infinite, perfectly conducting plane. It has no soil loss and needs no explicit radial geometry." : model.configuration === "elevated-explicit-radials" ? `${model.radials.count} radial wires are solved as conductors. ${model.ground.kind === "perfect" ? "The ground below is still an infinite perfect plane." : "Ground interaction uses NEC-2's Sommerfeld/Norton finite-ground method."}` : "Finite ground uses NEC-2's reflection-coefficient approximation plus the GN/RP radial-screen approximation. It is not Sommerfeld/Norton; screen currents and edge diffraction are not solved explicitly."}</div>
      </Card>
    </aside>
