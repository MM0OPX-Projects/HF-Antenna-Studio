...
function Scene({ generated }: { generated: GeneratedPhasedArray }) {
  const bounds = useMemo(() => {
    ...
    return { x: (Math.max(...xs) + Math.min(...xs)) / 2, y: (Math.max(...ys) + Math.min(...ys)) / 2, z: (Math.max(...zs) + Math.min(...zs)) / 2, scale: 6 / span };
  }, [generated]);
  ...
  return <>
    ...
    <OrbitControls makeDefault target={[0, bounds.z * bounds.scale, 0]} enablePan screenSpacePanning mouseButtons={{ LEFT: MOUSE.ROTATE, MIDDLE: MOUSE.PAN, RIGHT: MOUSE.PAN }} minDistance={0.2} maxDistance={30} />
  </>;
}

export function PhasedArrayGeometry3D({ generated, modelKey }: { generated: GeneratedPhasedArray; modelKey: string }) {
  return <div className="relative h-96 overflow-hidden rounded-md bg-[#07111f]" onWheel={preventViewportScroll} onAuxClick={preventViewportAutoScroll} data-testid="phased-array-geometry-3d" data-model-key={modelKey} data-wire-count={generated.wires.length}>
    <span className="sr-only">Interactive phased vertical array geometry. Orange is element one, cyan is element two, gray wires are explicit radials, yellow is the physical source junction, and dashed magenta paths are non-radiating ideal transmission lines. Drag to orbit, scroll to zoom, and middle/right-drag to pan.</span>
    <SafeCanvas camera={{ position: [7, 5, 8], fov: 43 }} dpr={[1, 1.5]}><Scene generated={generated} /></SafeCanvas>
    <div className="pointer-events-none absolute bottom-2 left-2 rounded bg-black/60 px-2 py-1 text-[10px] text-slate-200">E1 orange · E2 cyan · radials gray · TL paths dashed magenta · orbit · scroll zoom · middle/right-drag pan</div>
  </div>;
}
