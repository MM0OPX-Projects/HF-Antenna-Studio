...
import { activeMultibandHexbeamBand, loopBeamWavelengthM, multibandLongestBandExtraSpacingM, multibandStackSpacingM } from "./model";
...
export function adaptLoopBeamToNec(generated: GeneratedLoopBeamModel): AdaptedLoopBeamNec {
  ...
  const model = generated.model; const title = model.kind === "cubical-quad" ? "cubical quad" : model.kind === "hexbeam" ? "G3TXQ broadband Hexbeam" : model.kind === "multiband-hexbeam" ? "multiband Hexbeam" : model.kind.replace(/-/g, " "); const lines = [`CM HF Antenna Studio ${title}`, "CM SI units; every GW card is an actual solved conductor", `CM Feed-conductor orientation ${generated.feedConductorOrientation}; no polarisation claim is inferred`, ...(model.kind === "multiband-hexbeam" ? [`CM Selected bands ${model.bands.join(", ")}`, `CM Active feed band ${activeMultibandHexbeamBand(model)}; inactive driver gaps remain open`, `CM Vertical stack: lowest band height ${fmt(model.heightM)} m; centre spacing ${fmt(multibandStackSpacingM(model))} m; additional 17-20 spacing ${fmt(multibandLongestBandExtraSpacingM(model))} m; shortest wavelength at base`] : []), "CM Dimensions are starting points, not resonance or performance guarantees", "CE"];
