...
const HEX_REFERENCE_INCHES: Record<HexBand, { driverHalf: number; reflector: number; endSpacing: number; spreaderRadius: number }> = {
  "20m": { driverHalf: 218, reflector: 412, endSpacing: 24, spreaderRadius: 130 },
  "17m": { driverHalf: 169.5, reflector: 321, endSpacing: 18.5, spreaderRadius: 101 },
  "15m": { driverHalf: 144.5, reflector: 274.4, endSpacing: 16, spreaderRadius: 86 },
  "12m": { driverHalf: 121.7, reflector: 232, endSpacing: 13.5, spreaderRadius: 73 },
  "10m": { driverHalf: 106.8, reflector: 204.4, endSpacing: 12, spreaderRadius: 64 },
};
...
export function startingHexbeamModel(band: HexBand = "20m", frequencyHz?: number): HexbeamModel {
  ...
  return { ...common(requestedFrequency), kind: "hexbeam", band, drivenTotalLengthM: inches(ref.driverHalf * 2) * scale, reflectorTotalLengthM: inches(ref.reflector) * scale, endSpacingM: inches(ref.endSpacing) * scale, spreaderRadiusM: inches(ref.spreaderRadius) * scale, heightM: loopBeamWavelengthM(requestedFrequency) * 0.5, provenance: { dimensionsAreStartingPoints: true, manualDimensions: false, reference: "G3TXQ element/end-spacing facts; 20m radius published, smaller radii wavelength-scaled" } };
}
...
function hexWires(model: HexbeamModel) {
  const wires: LoopBeamWire[] = []; const supports: LoopBeamSupport[] = []; const lambda = loopBeamWavelengthM(model.frequencyHz); const z = model.heightM; const d = model.elementDiameterM; const r = model.spreaderRadiusM; const x = Math.sqrt(3) * r / 2; const frontY = r / 2; const feedBridge = Math.min(model.drivenTotalLengthM * 0.01, Math.max(lambda * 0.002, d * 6)); const feedLeft = { x: -feedBridge / 2, y: 0, z }; const feedRight = { x: feedBridge / 2, y: 0, z }; const leftBend = { x: -x, y: frontY, z }; const rightBend = { x, y: frontY, z }; const firstArm = distance(feedLeft, leftBend); const remainingDriver = Math.max(0, (model.drivenTotalLengthM - feedBridge) / 2 - firstArm); const leftEnd = { x: -x, y: frontY - remainingDriver, z }; const rightEnd = { x, y: frontY - remainingDriver, z };
  const feedWireId = "driven-feed"; wires.push({ id: feedWireId, family: "driven", startM: feedLeft, endM: feedRight, diameterM: d, source: true }); wire(wires, "driven-left-inner", "driven", feedLeft, leftBend, d); wire(wires, "driven-left-outer", "driven", leftBend, leftEnd, d); wire(wires, "driven-right-inner", "driven", feedRight, rightBend, d); wire(wires, "driven-right-outer", "driven", rightBend, rightEnd, d);
  const reflectorLeftEnd = { x: -x, y: leftEnd.y - model.endSpacingM, z }; const reflectorRightEnd = { x, y: rightEnd.y - model.endSpacingM, z }; const rearLeft = { x: -x, y: -frontY, z }; const halfTarget = model.reflectorTotalLengthM / 2; const firstReflector = distance(reflectorLeftEnd, rearLeft); const diagonal = halfTarget - firstReflector; const rearDepth = diagonal > x ? frontY + Math.sqrt(diagonal * diagonal - x * x) : r; const rear = { x: 0, y: -rearDepth, z };
  wire(wires, "reflector-left-tip", "reflector", reflectorLeftEnd, rearLeft, d); wire(wires, "reflector-left-rear", "reflector", rearLeft, rear, d); wire(wires, "reflector-right-rear", "reflector", rear, { x, y: -frontY, z }, d); wire(wires, "reflector-right-tip", "reflector", { x, y: -frontY, z }, reflectorRightEnd, d);
  const centre = { x: 0, y: 0, z }; const tipAngles = [30, 90, 150, 210, 270, 330]; for (const angle of tipAngles) { const radians = angle * Math.PI / 180; const effectiveRadius = angle === 270 ? rearDepth : r; supports.push({ id: `spreader-${angle}`, startM: centre, endM: { x: Math.cos(radians) * effectiveRadius, y: Math.sin(radians) * effectiveRadius, z } }); }
  return { wires, supports, feedWireId };
}
...
export function validateLoopBeamModel(model: LoopBeamModel, wires: LoopBeamWire[], feedWireId: string): LoopBeamIssue[] {
  ...
  if (model.kind === "hexbeam") { issues.push(...range(model.drivenTotalLengthM, 1, 100, "driver-length", "Driven-element length (m)"), ...range(model.reflectorTotalLengthM, 1, 100, "reflector-length", "Reflector length (m)"), ...range(model.endSpacingM, 0.02, 10, "end-spacing", "End spacing (m)"), ...range(model.spreaderRadiusM, 0.2, 30, "spreader-radius", "Spreader radius (m)"), ...range(model.heightM, 0.2, 100, "height", "Height (m)")); const source = wires.find((item) => item.id === feedWireId)!; const leftBend = wires.find((item) => item.id === "driven-left-inner")!; if (distance(source.startM, leftBend.endM) >= (model.drivenTotalLengthM - distance(source.startM, source.endM)) / 2) issues.push({ severity: "error", code: "hex-driver-fit", message: "Driven-element length is too short to reach the selected spreader geometry." }); const leftTip = wires.find((item) => item.id === "reflector-left-tip")!; const firstReflector = distance(leftTip.startM, leftTip.endM); const minimumRearLeg = Math.sqrt(3) * model.spreaderRadiusM / 2; if (model.reflectorTotalLengthM / 2 <= firstReflector + minimumRearLeg) issues.push({ severity: "error", code: "hex-reflector-fit", message: "Reflector length is too short to fit the selected driver tips, end spacing, and rear spreader path." }); if (wires.some((item) => ![item.startM.x, item.startM.y, item.startM.z, item.endM.x, item.endM.y, item.endM.z].every(Number.isFinite))) issues.push({ severity: "error", code: "hex-reflector-finite", message: "Reflector geometry contains a non-finite coordinate." }); issues.push({ severity: "warning", code: "hex-topology", message: "This is a single-band broadband-style wire topology. Support sag, insulation, mast/feed-line coupling, and multiband interaction are not modelled." }); }
