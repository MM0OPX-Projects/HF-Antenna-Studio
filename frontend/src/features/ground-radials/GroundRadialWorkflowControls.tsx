...
export function GroundRadialWorkflowControls({ settings, showVertical, showPhased, prefix, onChange }: Props) {
  ...
  return <div className="space-y-3 rounded border border-amber-500/30 bg-amber-500/5 p-3" data-testid={`${prefix}-radial-workflow`}>
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      ...
      {showPhased && settings.phasedMode !== "perfect-ground-image" && <NumberField label="Phased radial count" value={settings.phasedRadialCount} min={4} max={128} step={1} testId={`${prefix}-phased-radial-count`} onChange={(phasedRadialCount) => update({ phasedRadialCount: Math.round(phasedRadialCount) })} />}
