...
import { elevationCutFromPattern, radiationCutSeriesFromPattern } from "./radiation-cuts";
...
export function RadiationCutPair({
  ...
  const [sameBearing, setSameBearing] = useState(false);
  const [azimuthBearingDeg, setAzimuthBearingDeg] = useState(0);
  ...
  const elevation = sameBearing && pattern
    ? [{
      id: "current",
      label: "Current solved model",
      color: "#f97316",
      points: elevationCutFromPattern(pattern, azimuthBearingDeg, "legacy-compass"),
      current: true,
    } satisfies PolarSeries]
    : elevationSeries ?? derived?.elevation ?? [];
  const available = azimuth.length > 0 && elevation.length > 0;
  const cutContext = context
    ? `${context}${sameBearing ? ` · both cuts use ${azimuthBearingDeg.toFixed(1)}° compass bearing` : ""}`
    : derived
      ? `Azimuth starts at the strongest ${derived.azimuthElevationDeg.toFixed(1)}° NEC row and can be changed below · ${sameBearing ? `both cuts use ${azimuthBearingDeg.toFixed(1)}° compass bearing` : `elevation plane ${derived.elevationBearingDeg.toFixed(1)}° → ${((derived.elevationBearingDeg + 180) % 360).toFixed(1)}° bearing`}`
      : undefined;
  ...
  return <section className="space-y-2" data-testid={testId} aria-label={title}>
    <div className="flex flex-wrap items-start justify-between gap-2">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        <label className="flex items-center gap-1.5 rounded border border-border bg-surface px-2 py-1 text-[10px] text-text-secondary" title="Use the azimuth cursor bearing for the elevation cut as well">
          <input
            type="checkbox"
            checked={sameBearing}
            onChange={(event) => setSameBearing(event.currentTarget.checked)}
            data-testid={`${testId}-same-bearing`}
          />
          Same bearing for both cuts
        </label>
        <button
          type="button"
    ...
    {sameBearing && <p className="text-[10px] text-text-secondary" data-testid={`${testId}-same-bearing-help`}>Both plots use the azimuth cursor bearing; this is a same-geographic-direction comparison.</p>}
    ...
    {available && <div className={`grid gap-3 ${compact ? "lg:grid-cols-2" : "2xl:grid-cols-2"}`}>
      <Card className="overflow-hidden p-2" data-testid={`${testId}-azimuth`}><div className={compact ? "mx-auto w-full max-w-[500px]" : undefined}><HeightPolarPlot plane="azimuth" mode={mode} compactControls={compact} selectedAzimuthBearingDeg={azimuthBearingDeg} onSelectedAzimuthBearingChange={setAzimuthBearingDeg} series={pattern && !azimuthSeries ? azimuth.map((item) => ({ ...item, pattern, azimuthConvention: "legacy-compass" as const })) : azimuth} /></div></Card>
