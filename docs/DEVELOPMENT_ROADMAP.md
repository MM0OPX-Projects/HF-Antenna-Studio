...
Implementation checkpoint (2026-08-06): `feature/project-management` adds `.hfas` schema v4, immutable schema 1-4 migrations, browser-local named records, autosave, explicit recovery, recent/open/duplicate/rename/confirmed-delete operations, and portable import/export. This closes the browser-lifecycle subset only. Phase 4 remains open for complete runtime schema bounds, run manifests/cache identity, raw NEC project mode, native atomic filesystem writes/backups, Windows path/encoding tests, and dedicated laboratory-state inclusion.
