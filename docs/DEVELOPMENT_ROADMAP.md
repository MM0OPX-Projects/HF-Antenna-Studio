...
Implementation checkpoint (extended 2026-08-25): `feature/project-management` now writes `.hfas` schema v5, migrates schemas 1-4 immutably, and includes Template Simulator, Wire Editor, Model Comparison, Parameter Sweep, and Antenna Optimiser inputs in browser-local named records, autosave/recovery, and portable import/export. This closes those browser-lifecycle workflows only. Phase 4 remains open for complete runtime schema bounds, run manifests/cache identity, raw NEC and remaining laboratory project modes, native atomic filesystem writes/backups, and Windows path/encoding tests.
