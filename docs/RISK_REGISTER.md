...
|---|---|---|---|---|---|---|
| R-001 | The selected solver or integration gives numerically wrong results while producing plausible plots. | M / C | The verified 38 MHz free-space slice now matches a published external NEC-2 case, but ground and general models remain unverified. | Extend exact-deck external corpus, run 4NEC2 comparison, analytic bounds, convergence studies, and raw evidence. Block selection/release on unexplained deviations. | Validation lead | M / Mitigating |
...
| R-005 | Ground results are materially wrong for contact geometry or selected real-ground formulation. | M / C | Dedicated adapter emits explicit perfect/real GN cards and ground-height regressions run, but none has an independent numeric comparator yet. | Separate elevated/contact cases; original manual/reference decks; perfect/real-ground corpus; 4NEC2 settings review; visible formulation. | Validation lead | M / Open |
