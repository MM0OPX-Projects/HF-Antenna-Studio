...
|---|---|---|---|---|---|---|
| R-001 | The selected solver or integration gives numerically wrong results while producing plausible plots. | M / C | AntennaSim lacks real-solver tests; a parser fixture labelled as a free-space dipole expects physically implausible gain. | Exact-binary external corpus, historical NEC references, 4NEC2 comparison, analytic bounds, convergence studies, raw evidence. Block selection/release on unexplained deviations. | Validation lead | M / Open |
...
| R-004 | WebAssembly becomes a schedule distraction or ships with undetected parity problems. | H / H | AntennaSim Wasm only has smoke evidence; current NEC2++ Wasm entry point is a stub. | Defer Wasm until native validation exists; require exact corpus, warning, cancellation, and performance parity before enabling. | Architect | L / Mitigating |
| R-005 | Ground results are materially wrong for contact geometry or selected real-ground formulation. | M / C | GE/GN semantics depend on geometry and interpolation; AntennaSim always emits `GE 0` for grounded models. | Separate elevated/contact cases; original manual/reference decks; perfect/real-ground corpus; 4NEC2 settings review; visible formulation. | Validation lead | M / Open |
| R-006 | Theta/phi, compass, or cut-plane errors mirror/rotate radiation results. | H / H | AntennaSim uses negative theta domains and a maximum-containing row as “azimuth”; historical coordinate issues exist. | Canonical coordinate contract, axis unit tests, asymmetric Yagi/phased-array cases, raw grid inspection, explicit plane labels. | Visualization lead | L / Mitigating |
...
| R-010 | Solver warnings are lost, misparsed, or obscured by application heuristics. | H / H | Browser Wasm warnings are empty; backend recognizes only limited strings. | Retain complete raw output/stderr; versioned diagnostic extraction; warning corpus; separate solver and application origins; unknown-warning display. | Solver lead | M / Mitigating |
