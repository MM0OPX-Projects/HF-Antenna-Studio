...
- **Save** updates the named local project; **Save As** creates another record. When Save As is chosen from a workspace, Project Management opens in Save As mode, carries the live workspace snapshot with it, disables overwrite Save, and requires the separate Save As action.
...
- **Wire editor:** select a wire to open the Wire and Feedpoint Inspector. Edit X/Y/Z endpoints, length (keeping start, centre, or end fixed), bearing, elevation, diameter and segmentation in metric or imperial units. In the fixed 2D view, press and drag a wire body to reposition it directly while retaining its orientation. Drag either endpoint circle to reshape the wire with the opposite end anchored; bonded endpoints carry their neighbouring wire ends so a connected polyline deforms without silently disconnecting. Release creates one undo step and Escape/right-click cancels. Add mode can draw continuous joined wire chains; endpoint joining and Cartesian grid snapping are independent. Place a feedpoint by clicking/dragging along a wire or entering an exact percentage/distance. **New** (or Ctrl+N) starts a blank wire document after confirming unsaved changes; it clears geometry, sources, loads, transmission lines, junctions, radial systems, imports, selections and undo/redo history, resets numbering to Wire 1, detaches the previous project, and preserves display/model preferences. Always compare the requested point with the displayed actual NEC segment centre, and model an appropriate return path for end-fed systems.
- **Conductor material:** the global Copper, Aluminium, Steel, Stainless steel, Perfect conductor and Custom conductivity choices are representative bulk values applied through NEC `LD 5`. Steel and stainless values are alloy-dependent approximations; record the chosen value with results and see [`docs/CONDUCTOR_MATERIALS.md`](CONDUCTOR_MATERIALS.md) for sources and limitations.
...
- **Compare saved antennas:** each of the four Model Comparison source lists contains standard examples followed by eligible locally saved Design, Wire Editor, and antenna-module projects. Selecting one captures that exact saved revision inside the comparison project, so later rename, deletion, or editing of the library copy does not silently change a reproducible comparison. The common comparison frequency, ground, wire material and reference impedance are applied to a temporary copy only. Imported NEC files containing unsupported preserved cards are refused rather than approximated, and models with multiple enforced sources do not claim a single-port R, X or SWR.
