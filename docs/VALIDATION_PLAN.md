...
The later systematic campaign closes the established-package gap for the exact 0.5λ perfect-ground height case. The other height states and real-ground execution remain same-engine regression evidence only; consequently this checkpoint still does not satisfy the full release gate described below.
...
The eight default numeric cases are generated by the pinned application nec2c/Wasm engine. They detect changes but are not independent validation. Later family work and the systematic campaign add exact-deck external evidence for selected horizontal dipole, quarter-wave vertical, square-loop, and delta-loop states, but not for every template or configuration. Native/Wasm parity, segment and polygon convergence, real-ground/reference cases, and finer pattern sampling remain release gates. Full details and current numbers are in [`ANTENNA_TEMPLATE_SYSTEM.md`](ANTENNA_TEMPLATE_SYSTEM.md).
...
Implementation checkpoint (2026-08-06): model-comparison tests now prove complete condition/model identity gating, shared 10-degree NEC-grid cuts in compass coordinates, family metric mapping, mechanical pattern-deck-to-impedance-sweep conversion, and safe standalone report generation. A real-Wasm browser case solves one dipole, vertical, phased array, and Yagi under identical conditions. This is cross-feature integration evidence, not a new accuracy oracle: each metric inherits its family validation status, and comparison-specific release evidence still needs byte-identical native parity plus an established-package multi-model comparison at matching cuts and sweep points.

Implementation checkpoint (2026-08-06): parameter-sweep tests prove inclusive continuous/integer grids, exact coordinate-to-domain-model mapping for all eight registered parameters, bounded 1D/2D job plans, atomic cancellation, exact-model cache identity, point/deck fingerprints, and complete JSON lineage. Real-Wasm browser cases solve three dipole-height points and a four-cell vertical-length/radial-count grid. This establishes orchestration and provenance only. Before release, select boundary/interior points from every registered parameter and compare the exported byte-identical decks/results with ordinary native runs and at least one established package; add segmentation/topology discontinuity and packaged Windows workload evidence.

Implementation checkpoint (2026-08-06): antenna-optimiser pure tests use a known convex fixture to verify score improvement, deterministic candidate history, all objective formulas, constraints/rejections, exact cache/deck identity, cancellation, and non-global export claims. Real-Wasm browser cases execute a seven-evaluation dipole SWR search and a constrained two-variable Yagi weighted search, then inspect starting/best/retained evidence and cache replay. These are optimiser-orchestration tests, not validation that the selected NEC candidate is physically optimal. Before supported use, independently rerun sampled start/intermediate/best candidates through the native runner and an established package, test segment/grid convergence and parameter perturbations around the winner, and demonstrate that invalid/discontinuous candidates cannot game every supported objective.

Implementation checkpoint (2026-08-06): measurement-comparison tests prove bounded Touchstone 1.0/conservative 2.0 RI/MA/DB parsing, exact source-line retention, S11-to-Z/SWR derivation, malformed/ambiguous input rejection, exact matching, simulation-only R/X interpolation, no extrapolation, unlike-reference SWR gating, explicit non-finite export, cancellation and source-labelled plots. A real-Wasm browser case compares an application-created five-point NanoVNA-style fixture with an NEC impedance sweep. This is parser/orchestration evidence, not a controlled measurement validation. Release evidence requires independently calculated parser fixtures, identified NanoVNA-Saver/NanoVNA-QT exports, calibrated reference-plane measurements, characterised-feed comparisons, dense-grid interpolation bounds, native parity and a second RF-tool calculation.
