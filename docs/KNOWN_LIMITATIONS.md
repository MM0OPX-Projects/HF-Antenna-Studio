...
- **Ground-mounted radial systems:** v1.0.0 does not provide a verified general-purpose model for explicit radial wires lying on or buried in real earth. The single-vertical laboratory offers an NEC radial-screen approximation, but those “radials” are not current-carrying wire geometry and that finite-ground approximation lacks an independent v1 numeric campaign. Ground-contact perfect-ground single and phased verticals contain no radial wires. Explicit current-carrying radials are limited to supported elevated arrangements. These cases are physically different and must not be treated as interchangeable.
