...
- The first isolated Windows run then made the remaining limit explicit: the two tests which intentionally iterate every template exhausted the generic two-minute whole-test budget, while all five later template cases passed. Those two aggregate tests retain their per-template geometry, feed, segmentation, NEC, solver, numeric-result, range and slider assertions, but receive a ten-minute budget only on Windows CI; local and Linux runs retain the two-minute default. This accommodates the hosted runner's software-rendered 3D/Wasm throughput without accepting a wrong result or unbounded hang.
