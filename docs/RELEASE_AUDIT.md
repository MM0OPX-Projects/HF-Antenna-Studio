...
| Unit/type/lint/build | Fresh `npm ci`, type-check, ESLint, complete Vitest, production Wasm build | Pass: 646 tests; lint 0 errors/13 reviewed warnings |
| Integration/UI | Complete Playwright suite, browser-console and every-route return audit, stale/cancel/error paths | Pass locally: 94/94 plus 120/120 repeated navigation stress cases; exact final-tree CI required |
...
- Vite reports the main minified chunk at approximately 771.21 kB against the project's 750 kB advisory threshold. The application uses route-level lazy chunks for major laboratories; further splitting is a post-v1 startup-performance task. The build succeeds and installed-package responsiveness remains part of Windows acceptance.
...
- A later complete local rerun caught an intermittent React Three Fiber lifecycle failure while rapidly leaving Height Lab: asynchronous canvas setup could finish after the route had unmounted and try to connect pointer events to a null DOM target. One shared guarded canvas now retains the standard pointer manager while ignoring only that stale null-target connection across every 3D viewer. The new regression performs 12 rapid Height Lab enter/return cycles with console and page-error assertions; five full repetitions passed 120/120 navigation cases. The complete suite now contains 94 behavioural cases and must pass again on the exact final tree.
...
- Once the aggregate template suite completed on hosted Windows, the independent baseline file exposed the same whole-test coupling: the dipole had not published its result before the generic 120-second test deadline, while the vertical had published a result and visible Three.js canvas but exhausted that deadline while Playwright queried the canvas bounds; the Yagi completed. Raising result publication to four minutes was still a noisy throughput gate: on the next clean attempt the dipole exceeded four minutes while vertical/Yagi passed, whereas one unchanged diagnostic rerun passed dipole/Yagi and exceeded four minutes for vertical; the all-template file itself varied from 6.0 to 26.7 minutes while passing identical assertions. The three baseline cases retain their recorded SWR/R/X/gain envelopes, visible 3D canvas and dimensions, pattern/impedance tabs, and console/page-error assertions. Windows CI alone therefore uses a ten-minute result-publication bound and a twelve-minute whole-test bound, with a separate 120-minute cap on the complete Windows job; local and Linux retain the two-minute bounds. A retried run is diagnostic evidence only and is not accepted as the final release gate.
